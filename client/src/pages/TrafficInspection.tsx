...
export default function TrafficInspection() {
  ...
  const filteredTraffic = trafficData.filter(traffic => {
    const matchesSearch = traffic.sourceIp.includes(searchQuery) ||
  ...
  return (
    <div className="p-6 space-y-6 bg-background min-h-screen">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Network Traffic Inspection</h1>
          <p className="text-muted-foreground">Analyze real-time network traffic, inspect packet details, and apply advanced filters to diagnose anomalies and secure your network</p>
        </div>
      </div>
      ...
      <Card className="p-6 bg-card shadow-card">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold text-foreground">Traffic Filters & Actions</h3>
        </div>
        ...
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-semibold text-foreground">Live Traffic Log</h3>
              {isCapturing && (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border bg-muted/20">
                ...
                <th className="text-left p-4 font-medium text-foreground">Source IP:Port</th>
                <th className="text-left p-4 font-medium text-foreground">Destination IP:Port</th>
                <th className="text-left p-4 font-medium text-foreground">Protocol</th>
                <th className="text-left p-4 font-medium text-foreground">L7 App</th>
                <th className="text-left p-4 font-medium text-foreground">Payload Preview</th>
                <th className="text-left p-4 font-medium text-foreground">Session ID</th>
            ...
            <tbody>
              {filteredTraffic.map((traffic, index) => (
                <tr key={index} className="border-b border-border hover:bg-muted/10 transition-colors">
                  <td className="p-4">
                    <span className="font-mono text-sm text-foreground">{traffic.timestamp}</span>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <StatusBadge 
                      variant={traffic.protocol === "TCP" ? "info" : traffic.protocol === "UDP" ? "warning" : "default"}
                    >
                      {traffic.protocol}
                    </StatusBadge>
                  </td>
                  <td className="p-4">
                    <StatusBadge 
                      variant={
                        traffic.l7App === "HTTPS" ? "success" : 
                        traffic.l7App === "HTTP" ? "info" :
                        traffic.l7App === "SSH" ? "medium" :
                        traffic.l7App === "DNS" ? "low" : "default"
                      }
                    >
                      {traffic.l7App}
                    </StatusBadge>
                  ...
                  <td className="p-4">
                    <span className="font-mono text-sm text-muted-foreground">{traffic.sessionId}</span>
                  </td>
                  <td className="p-4">
                    <Button variant="ghost" size="sm">
